...
schedule 事件的去重只能在真实调度里观察:下一个工作日 09:23 左右那次运行,如果主链(8:57)已经成功,它的"检查新债并推送"步骤应该显示为 **skipped**;若主链没成功,该步骤会执行并补推。

- [ ] **Step 6: 确认下个交易日早上 8:57 收到推送**

Expected: 北京时间 8:57 左右收到新债提醒,且当天没有重复推送。

---

## 完成标准

- [ ] `PYTHONUTF8=1 python test_pipeline.py` 输出 `61 passed, 0 failed`
- [ ] 6 个提交都在 `master` 上
- [ ] 主链(Cloudflare)`wrangler deploy` 后 cron 为 `57 0 * * *`
- [ ] 工作日早上 8:57 收到推送,且当天只有一条
- [ ] 人为让主链失效时,09:11 的看门狗能发出微信告警
